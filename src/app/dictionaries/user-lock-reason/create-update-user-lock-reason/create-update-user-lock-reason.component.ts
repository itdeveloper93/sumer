...
export class CreateUpdateUserLockReasonComponent implements OnInit {
    ...
    createUpdateUserLockReason = new FormGroup({
        id: new FormControl(''),
    ...
    ngOnInit() {
        if (this.data.id) {
            ...
            this.dictionarieService.getDictionariesSubValuesById(this.data.id, 'UserLockReason').subscribe(
                res => {
                    this.createUpdateUserLockReason.patchValue({
                        id: this.data.id,
                        name: this.data.name,
    ...
    onSubmit() {
        if (this.createUpdateUserLockReason.invalid) {
        ...
        if (this.data.id) {
            ...
            this.dictionarieService
                .updateDictionariesSubValues(this.createUpdateUserLockReason.value, 'UserLockReason')
                .subscribe(
                    response => {
            ...
            this.isRequesting = true;
            const { name, isActive } = this.createUpdateUserLockReason.value;
